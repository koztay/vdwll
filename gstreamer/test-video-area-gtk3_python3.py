...
class Video:
    ...
    def __init__(self):
        ...
        print
        'Auto flush:', self.player.get_auto_flush_bus()
        self.player.set_property('auto-flush-bus', False)  # set_auto_flush_bus()
        print
        'Delay:', self.player.get_delay()
    ...
    def create_player(self):
        if Gst.ElementFactory.find('playbin'):  # filesrc
            ...
            print
            'Error: could not find the playbin plugin for gstreamer'
            sys.exit(1)

        if self.player == None:
            print
            'Error: could not create the player'
            sys.exit(1)
        else:
            print
            '\nLoaded:', self.player.get_name(),
            print
            'from', self.player.get_path_string()

            print
            '  Base time:', self.player.get_base_time()
            print
            '  Control rate:', self.player.get_control_rate()
            print
            '  Delay:', self.player.get_delay()
            print
            '  Parent:', self.player.get_parent()
            print
            '  Start time:', self.player.get_start_time()
            print
            '  Children:', self.player.numchildren
            print
            '  Pads:', self.player.numpads
            print
            '  Sinkpads:', self.player.numsinkpads
            print
            '  Srcpads:', self.player.numsrcpads
            ...
            if not factory.get_element_type().name == 'GstPlayBin':
                print
                'Error: player is not a GstPlayBin object'
                sys.exit(1)
            if factory.check_version(1, 0, 0):  # Check the version for playbin
                print
                '\n  Loaded factory:', factory.get_name(),  # playbin
                print
                'from', factory.get_path_string()  # /GstRegistry:registry0/GstElementFactory:playbin

                print
                '    Pad templates:', factory.get_num_pad_templates()  # 0
                print
                '    Rank:', factory.get_rank()  # 0
                print
                '    Control rate:', factory.get_control_rate()  # 100000000
                # print element.get_uri_protocols() # []
                # print element.get_uri_type() # <enum GST_URI_UNKNOWN of type URIType>
            else:
                print
                'Player does not have the required version.'
                sys.exit(1)
    ...
    def on_message_new_clock(self, bus, message):
        ...
        # GStreamer-CRITICAL **: gst_message_parse_qos: assertion `GST_MESSAGE_TYPE (message) == GST_MESSAGE_QOS' failed
        # (False, 0L, 0L, 0L, 0L)
        print(message.parse_qos_stats())
        # GStreamer-CRITICAL **: gst_message_parse_qos_stats: assertion `GST_MESSAGE_TYPE (message) == GST_MESSAGE_QOS' failed
        # (<enum GST_FORMAT_UNDEFINED of type Format>, 0L, 0L)
        print(message.parse_qos_values())
        # GStreamer-CRITICAL **: gst_message_parse_qos_values: assertion `GST_MESSAGE_TYPE (message) == GST_MESSAGE_QOS' failed
        print(message.parse_request_state())
        # GStreamer-CRITICAL **: gst_message_parse_request_state: assertion `GST_MESSAGE_TYPE (message) == GST_MESSAGE_REQUEST_STATE' failed
    ...
    def on_message_error(self, bus, message):
        ...
        if message.type == Gst.MessageType.ERROR:
            ...
            print('First value:', message.type.first_value_name)
            # 'first_value_name', 'first_value_nick', 'imag', 'numerator', 'real', 'value_names', 'value_nicks']
    ...
    def on_sync_message(self, bus, message):
        ...
        if structure_name == 'playbin-stream-changed':
            ...
            print('\n  Loaded factory:', factory.get_name())
            # xvimagesink
            print('from', factory.get_path_string())
            # /GstRegistry:registry0/GstElementFactory:xvimagesink
            print(dir(factory))
            ...
            imagesink.set_property('double-buffer',
                                   True)  # Gdk-WARNING **: XSetErrorHandler() called with a GDK error trap pushed. Don't do that.
    ...
    def configure_bus(self):
        ...
        print('\nLoaded factory:', self.bus.get_name())
        # bus
        print('from', self.bus.get_path_string())
        # /GstBus:bus1
