...
class AudioPlayer:
    ...
    def constructPipeline(self):
        ...
        self.filesrc.set_property("location", "/Users/kemal/WorkSpace/Videowall Development/media/si_se_the_truth.mp3")
    ...
    def play(self):
        ...
        while self.is_playing:
            time.sleep(0.5)
            try:
                self.position = self.player.query_position(Gst.Format.TIME)
            ...
            if not self.position is None:
                ...
                self.position = (True, self.position[1] / Gst.SECOND)
    ...
    def okToRunExamples(self):
        ...
        if self.duration is None:
            time.sleep(0.2)
            try:
                self.duration = self.player.query_duration(Gst.Format.TIME)
                # Convert the duration into seconds.
                self.duration = (True, self.duration[1] / Gst.SECOND)
            except Gst.QueryError:
                print("\n ERROR: unable to determine duration."
                      "Example not run.")
                return False

        if self.duration[1] < 20:
